...
function Contact({ onQuote }) {
  ...
  const goRoute = (id) => {
    if (id === 'quote') { onQuote && onQuote(); return; }
    setFocused(id);
    const el = routesRef.current;
    if (el && el.scrollIntoView) el.scrollIntoView({ behavior: 'smooth', block: 'center' });
    window.clearTimeout(goRoute._t);
    goRoute._t = window.setTimeout(() => setFocused(null), 2600);
  };
  return (
    <div>
      ...
      <Section tight>
        <Page>
          <div className="ubc-contact-grid" style={{ display: 'grid', gridTemplateColumns: '.85fr 1.15fr', gap: 'var(--s-8)', alignItems: 'start' }}>
            ...
            {/* routes + form */}
            <div>
              <div ref={routesRef} className="ubc-route-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--s-4)' }}>
                {[
                  ...
                  <div key={r.head} style={{ background: 'var(--surface-card)', border: 'var(--bw-hair) solid ' + (focused === r.id ? 'var(--accent)' : 'var(--border-subtle)'), borderRadius: 'var(--r-2)', padding: 'var(--s-5)', boxShadow: focused === r.id ? '0 0 0 4px var(--accent-tint), var(--shadow-2)' : 'var(--shadow-1)', transition: 'border-color var(--dur-2) var(--ease-out), box-shadow var(--dur-2) var(--ease-out)' }}>
