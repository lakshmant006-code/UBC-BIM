...
<rule id="motion" enforce="strong">
  ...
  <accessibility>prefers-reduced-motion collapses --dur-3/4/cine to 0 and renders layers in final position; the anime.js-driven reveals, counters, accordion and button bounce all check prefers-reduced-motion in JS and skip straight to the end state. The globe's marker pulses check the same media query directly (the render loop isn't anime.js-driven) and fall back to steady, non-pulsing dots.</accessibility>
</rule>
