...
<rule id="motion" enforce="strong">
  ...
  <exception>The Testimonials section (Home) is a four-column, continuously-scrolling marquee, tilted in 3D — a direct, explicit request for exactly this shape of component, not a house-style default. Unlike every other exception on this rule, this genuinely is the "carousels" ban's actual target (an unattended, looping slideshow) and is kept anyway because it was asked for by name; it does not pretend to be something milder. Pauses per-column on hover, and prefers-reduced-motion stops it outright (`animation: none`, not just a slower duration) rather than leaving a softened version running.</exception>
  <exception>The Global Presence globe (Home) marks eight regions (Canada, USA, Europe, Israel, Russia, India, Australia, New Zealand) with a looping marker-size pulse, staggered per marker. This is a direct, explicit request ("mark X blinking"), not decoration invented in-house, and it is legend-like rather than atmospheric: each pulse points at a specific, named piece of information (a served region) the way a map's blinking pin does, not an idle background loop with nothing to say.</exception>
  <exception>That same globe also turns on its own — a slow, constant longitude drift that pauses the instant someone starts dragging it and resumes the instant they let go. A second explicit request ("give it circular motion when still"), not the first version's own idea: this card was originally built drag-only specifically to avoid ambient auto-rotate, and only turns it on now because it was asked for directly, the same standard the marker pulse above was held to.</exception>
  <exception>The Mocking Bird Lot 2 model page (services route) marks five real structural details — corner stud, hold-down, anchor bolt, roof beam, a bracing candidate — with looping red pulsing markers ("red blinkers"), a direct, explicit request, not decoration invented in-house. Same standard as the globe's marker pulse: legend-like, not atmospheric — each one points at a specific named detail a visitor can click to read about, not an idle loop with nothing to say.</exception>
  ...
  <accessibility>prefers-reduced-motion collapses --dur-3/4/cine to 0 and renders layers in final position; the anime.js-driven reveals, counters, accordion and button bounce all check prefers-reduced-motion in JS and skip straight to the end state. The globe's marker pulses and its auto-rotate both check the same media query directly (the render loop isn't anime.js-driven) and fall back to steady, non-pulsing dots and a fixed, non-rotating orientation respectively. The testimonials marquee checks it in JS too, setting `animation: none` on every column rather than leaving a slower loop running. The Mocking Bird Lot 2 hotspot markers' ring pulse is plain CSS, so it's caught the same way: `animation: none` under the media query, left as a steady ring rather than removed outright, so the marker itself is still findable.</accessibility>
</rule>
